...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        for g in newGhostStates:
            ...
            ghost_p.append(distance)
        for d in newFood:
            distance = manhattanDistance(newPos, d)
            food_p.append(distance)      

        if newScaredTimes[0] > 0:
            return 10 + successorGameState.getScore()
        else:
            if len(ghost_p) > 0 and len(food_p) > 0:
                finalS = successorGameState.getScore()+min(ghost_p) - min(food_p)
                if min(food_p) == 1 and min(ghost_p) == 1:
                    return -1
                else:
                    return finalS
            else:
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        # after that depth
        def after_depth_action(gameState, agentIndex, depth):
            lastScore = 0
            ...
            if agentIndex == 0:
                ...
                temp = -1000
                for a in actions:
                    next_val = after_depth_action(gameState.generateSuccessor(agentIndex, a), agentIndex +1, depth)
                    if next_val > temp:
                        temp = next_val
                        lastScore = temp
            else:
                ...
                if agentIndex == number_of_agents -1:
                    for a in actions:
                        next_val = after_depth_action(gameState.generateSuccessor(agentIndex, a),agentIndex + 1, depth + 1)
                        if next_val < temp:
                            temp = next_val
                            lastScore = temp
        ...
        for a in gameState.getLegalActions(0):
            temp = after_depth_action(gameState.generateSuccessor(0, a), 1, 0)
            if temp > best_score:
                best_score = temp
                next_action = a
        return next_action
        ...
        def after_depth_action(gameState, agentIndex, depth):
            ...
            if agentIndex == 0:
                ...
                for action in actions:
                    next_val = after_depth_action(gameState.generateSuccessor(agentIndex, action), agentIndex + 1, depth)
                    if next_val > temp:
                        temp = next_val
                        lastScore = temp
            else:
                ...
                if agentIndex == number_of_agents - 1:
                    for action in actions:
                        next_val = after_depth_action(gameState.generateSuccessor(agentIndex, action), agentIndex + 1, depth + 1)
                        # only changed part is the following (in comparison to minimax agent):
                        temp += next_val / len(actions)
                    lastScore = temp
...
def betterEvaluationFunction(currentGameState):
    ...
    remainingF  = currentGameState.getFood()
    gStates = currentGameState.getGhostStates()

    score = currentGameState.getScore()               

    dis=[]
    for f in remainingF.asList()   :
        disToFood = manhattanDistance( pPos , f   )
        dis.append(disToFood)

    if not len( dis ) == 0   :
        score =(score+(10.0 /min(dis)))
